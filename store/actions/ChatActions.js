...
export const newChatRoom = (chatroomName) => {
    return async (dispatch, getState) => { // redux thunk
        ...
        const response = await fetch('https://kvaliapp-baa85-default-rtdb.europe-west1.firebasedatabase.app/chatrooms.json?auth=' +  token, {
            method: 'POST',
            headers: {
...
export const newChatMessage = (chatRoomId, message) => {
    return async (dispatch, getState) => { // redux thunk
        ...
        // console.log("message", message);
                                    // Find this link for YOUR firebase, in the "Realtime Database"-tab in the firebase console
                                    // You must use YOUR link and not this link, to save data in your database and not mine.
        const response = await fetch('https://kvaliapp-baa85-default-rtdb.europe-west1.firebasedatabase.app/chatrooms/' + chatRoomId + '/messages.json?auth=' +  token, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
